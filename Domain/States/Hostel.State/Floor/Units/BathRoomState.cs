...
namespace Hostel.State.Floor.Units
{
    public class BathRoomState : Message, IState<BathRoomState>
    {
        ...
        public BathRoomState(string id, string tag, IEnumerable<SensorSpec> sensors) : this(id, tag, sensors, Enumerable.Empty<Reading>(), Enumerable.Empty<Reading>())
        {
            Tag = tag;
            Sensors = sensors;
            BathRoomId = id;
        }
        public BathRoomState(string id, string tag, IEnumerable<SensorSpec> sensors, IEnumerable<Reading> current, IEnumerable<Reading> previous)
        {
            Tag = tag;
            Sensors = sensors;
            BathRoomId = id;
